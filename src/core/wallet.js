...
class WalletManager extends Client {
  ...
  async connect() {
    try {
      ...
      const accountType = "private"; // Assuming private key type
  ...
  async startMatch(agent, session) {
    try {
      ...
      if (isInQueue) {
        Tools.log(`Agent ${agent.name} is already in queue. Skipping...`);
      ...
      const captchaSolutionUpper = captchaSolution.toUpperCase(); // Convert to uppercase
      ...
      const response = await this.fetch(
        ...
        this.token,
        {
          ...
          captchaText: captchaSolutionUpper, // Use uppercase captcha solution
        }
      );
      ...
      if (response.status === 200) {
        ...
        this.activeMatches[agent.id] = true;
        Tools.updateDisplay(this);
        Tools.log(`Agent ${agent.name} is already in active match. Skipping...`);
        return false; // Skip this agent
      ...
      } else if (response.data?.error?.includes("maximum number of sessions")) {
        throw new Error("maximum number of sessions"); // Throw error to trigger session limit handling
      } else {
        Tools.log(`Failed to start match: ${response.data?.error || "Unknown error"}`);
        throw new Error("Failed to start match"); // Throw error to trigger retry
      }
    } catch (error) {
  ...
  async estimateGas(txData, value = 0, isFeatureEnabled = false, maxRetries = 3) {
